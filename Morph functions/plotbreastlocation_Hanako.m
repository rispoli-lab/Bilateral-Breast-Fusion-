...
[columnsInImage rowsInImage] = meshgrid(1:size(ZProject,1), 1:size(ZProject,2));
...
rectangle('Position',[47,140,20,39],'FaceColor','w');set(gca,'YDir','normal');hold on;%cover face
...
circlePixels1 = 0;
for h1=1:2:length(contour1_Z(:,2))
    circlePixels2 = (columnsInImage - contour1_Z(h1,2)).^2 ...
    + (rowsInImage - contour1_Z(h1,1)).^2 <= (24/4).^2 ;
...
circlePixels1 = 0;
for h1=1:2:length(contour2_Z(:,2))
    circlePixels2 = ( columnsInImage- contour2_Z(h1,2)).^2 ...
    + (rowsInImage - contour2_Z(h1,1)).^2 <= (24/4).^2 ;
    circlePixels1 = circlePixels2 + circlePixels1;
