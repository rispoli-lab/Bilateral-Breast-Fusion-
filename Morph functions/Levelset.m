...
function [] = Levelset(BreastExtrusion,loadmodel,start,finish)

% BreastExtrusion(341,79,end-6) = 23; % defect of model
path2 = 'C:\Users\xin\Documents\Breast Morph\version 5\MATLAB\Scripts Version 12_8_2017\Levelset_figures';
image1 = squeeze(BreastExtrusion(:,190,:));
image1(image1 == 27) = 60; %skin
...
image2 = squeeze(BreastExtrusion(:,:,199));
...
figure3 = figure(91);imagesc(  imfill(squeeze(Breastboundary(:,190,:)), 'holes') );axis equal; axis tight;title('Joint with un-natural curvature');set(gca,'FontSize', 18);
...
figure(1007);imagesc(squeeze(Signeddistance(:,190,:)));axis equal; axis tight;
...
display('Applying level-set method ...')
 tic
 [ data, g, data0 ] = LSinp('low',Signeddistance,4,1/50);%9 for NICT model. call level set function.the level set result is stored in data 
 toc
% tic
...
figure4 = figure(1001);imagesc(squeeze(smoothedjoint(:,190,:)));axis equal; axis tight;title('Joint with natural curvature');set(gca,'FontSize', 18);
    ...
    for j = 2:size(BreastExtrusion,2)-1
        for k = 2:size(BreastExtrusion,3)-1
            ...
            if smoothedjoint(i-start+1,j,k)==1 
                ...
                if smoothedbreastchest(i,j,k)== 27 
...
% figure(1002);imagesc(squeeze(smoothedbreastchest(:,190,:)));axis equal; axis tight;colormap(hot(100));caxis([0 101]);

takeoutskin = smoothedbreastchest(start-50:start+1,:,:);
takeoutskin(takeoutskin==27) = 101; % subcutaneouse fat 30
smoothedbreastchest(start-50:start+1,:,:) = takeoutskin;

%
for i = start-50:finish
    for j = 1:size(smoothedbreastchest,2)
        for k = 1:size(smoothedbreastchest,3)
   
              if smoothedbreastchest(i,j,k) ~= 0
                  
                  if j == 1 || j == size(smoothedbreastchest,2) || k == 1 || k == size(smoothedbreastchest,3)
                    
                      for n1 = -1:1:1 
                        if smoothedbreastchest(i + n1  ,j  ,  k )== 0 
                            smoothedbreastchest(i,j,k) = 86;
                        end
                      end
                    continue
                 ...
                 if i == start-50 || i == finish
                    
                     
                     for n2 = -1:1:1
                        for n3 = -1:1:1
                                if smoothedbreastchest(i , j+n2 ,  k+n3 )== 0 
                                     smoothedbreastchest(i,j,k) = 86;
                                end
                        end
                    end
...
%
image1 = squeeze(smoothedbreastchest(:,190,:));
image1(image1 == 43) = 11;
image1(image1 == 14) = 43;
image1(image1 == 27) = 60; %skin
image1(image1 == 100 | image1 == 101) = 30; %subcutaneouse fat
...
path = ['C:\Users\xin\Documents\Breast Morph\version 5\MATLAB\Scripts Version 12_8_2017\Outputmodel\ForElla\'];
WriteToVOX( [name,'.vox'], Croppedbreastchest, path);%[ ] = writeToVOX( filename, outputData, pathname)
WriteTommf( name, filesize, path,0.5);%[ ] = writeToVOX( filename, outputData, pathname)
end