...
function BreastExtrusion = BreastExtrusion_Ella(Breastdata, Trunk, ClosePec, Chestdataorigion,Breastorigion)
...
Chestdata = ClosePec(1:end-150,relativeorigion(1):relativeorigion(1)+size(Breastdata,2)-1,relativeorigion(2):size(Breastdata,3)+relativeorigion(2)-1);
...
%figure(1009); imagesc(squeeze(sum(Chestdata,1)));axis equal; axis tight;

AddSpace_breast = zeros(400+size(Breastdata,1),size(Breastdata,2),size(Breastdata,3)); % increase more space for the breast, the first dim is breast height
...
% figure(1009257);imagesc(squeeze(Curvedbreast(:,100,:)));axis equal; axis tight;
    ...
    for j = 1:size(Curvedbreast,2);
        for k = 1:size(Curvedbreast,3); 
                    ...
                    if Curvedbreast(i,j,k) ~=0 
                        ...
                        if Trunk(199+i, relativeorigion(1)-1+j,relativeorigion(2)-1+k) == 43 && Curvedbreast(i,j,k) ~=86
                                    Trunk(199+i, relativeorigion(1)-1+j,relativeorigion(2)-1+k) = Curvedbreast(i,j,k);
                        end
                    end  
        ...
        for k = 1:size(Curvedbreast,3); 
                    ...
                    if Curvedbreast(i,j,k) ~=0 
                        if  (Curvedbreast(i,j,k) ~= 86) || ...
                            (Curvedbreast(i,j,k) == 86 && Trunk(199+i, relativeorigion(1)-1+j,relativeorigion(2)-1+k) == 0)
                                    Trunk(199+i, relativeorigion(1)-1+j,relativeorigion(2)-1+k) = Curvedbreast(i,j,k);
                        end
                    end  
